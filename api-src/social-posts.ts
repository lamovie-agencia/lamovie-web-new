...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const result = await db.query('SELECT * FROM social_posts ORDER BY date DESC, id DESC');
      return res.status(200).json(result.rows);
    }

    if (req.method === 'POST') {
      const { text, platform, date, status } = req.body || {};
      if (!text) return res.status(400).json({ error: 'Text is required' });

      const result = await db.query(
        `INSERT INTO social_posts (text, platform, date, status)
         VALUES ($1, $2, $3, $4)
         RETURNING *`,
        [text, platform || 'Instagram', date || '', status || 'Programado']
      );
