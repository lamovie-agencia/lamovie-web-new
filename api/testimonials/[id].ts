...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'PUT' || req.method === 'PATCH') {
      ...
      const result = await db.query(
        `UPDATE testimonials
         ...
         WHERE id = $6
         RETURNING *`,
        [firstDefined(body.name), firstDefined(body.role), firstDefined(body.content), firstDefined(body.image_url), body.rating === undefined ? null : Number(body.rating), id]
      );
    ...
    console.error('Testimonial item API error:', error);
